...
class AllPass1stCell_SW(Module):
    def __init__(self):
        super(AllPass1stCell_SW, self).__init__()
    ...
    def init_states(self, size):
        v = torch.zeros(size, 4).to(next(self.parameters()).device)
        return v
    ...
    def forward(self, input, v):
        ...
        output = ( (self.a + self.g) * input + v[:,3]) / (1.0 + self.a * self.g)
        v[:, 3] = v[:, 2]
        v[:, 2] = v[:, 1]
        v[:, 1] = v[:, 0]
        v[:, 0] = (1.0 + self.a * self.g) * input - (self.a + self.g) * output
        
        return output, v

class AllPass1stOrder_SW(Module):
    def __init__(self):
        super(AllPass1stOrder_SW, self).__init__()
        self.cell = AllPass1stCell_SW()
