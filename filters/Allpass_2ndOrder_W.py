...
class AllPass2ndCell_W(Module):
    def __init__(self):
        super(AllPass2ndCell_W, self).__init__()
    ...
    def forward(self, input, v):    
        ...
        output = (input * (self.c + self.a * self.a + self.a * self.d) + v[:, 0]) / (1.0 + self.a * self.a * self.c + self.a * self.d)
        
        v[:, 0] = input * (self.a + self.a + self.d + self.a * self.c) + \
        output * (-self.a * self.c - self.a * self.c - self.d - self.a) + \
        (self.a * (input - self.c * output)) * (-self.a * self.a) + (-self.a * self.a + 1.0) * v[:, 1].clone()
        
        v[:, 1] = input - self.c * output - self.a * (self.a * (input - self.c * output) + v[:, 1])
        
        v = self._cat([v[:, 0], v[:, 1]])
        
        return output, v

class AllPass2ndOrder_W(Module):
    def __init__(self):
        super(AllPass2ndOrder_W, self).__init__()
        self.cell = AllPass2ndCell_W()
